...
type RelatorioMensal = {
  dataFechamento: string; // Ou Date, dependendo de como você serializa/desserializa
  totalVendas: number;
  lucroBruto: number;
...
export default function Dashboard() {
  ...
  const handleConfirmFecharMes = async () => {
    ...
    setIsLoadingFecharMes(true); // Ativa o estado de carregamento
    ...
    try {
      ...
      console.log("Dados de relatório recebidos do backend:", data); // LOG PARA DEBUG

      // Garante que data.relatorio existe e é do tipo correto
      if (data && data.relatorio) {
        setRelatorioMensal(data.relatorio);
        setIsModalOpen(true); // Abre o modal de relatório
        console.log("Modal de relatório SETADO para abrir."); // LOG PARA DEBUG
      } else {
        throw new Error("Resposta do backend não contém o relatório esperado.");
      }


      // Após fechar o mês com sucesso, recarregar os dados do dashboard
      const updatedDashboardHeaders: HeadersInit = {};
      if (token) {
        updatedDashboardHeaders['Authorization'] = `Bearer ${token}`;
      }
      // Usando async/await para recarregar o dashboard também para melhor tratamento de erros
      try {
        const dashboardRes = await fetch(`${API_BASE_URL}/dashboard`, { headers: updatedDashboardHeaders });
        if (!dashboardRes.ok) {
          throw new Error(`HTTP error! status: ${dashboardRes.status}`);
        }
        const newDashboardData = await dashboardRes.json();
        setDados(newDashboardData);
        console.log("Dashboard recarregado após fechamento do mês.");
      } catch (error) {
        console.error("Erro ao recarregar dashboard após fechamento:", error);
        setErrorMessage("Erro ao recarregar os dados do dashboard após o fechamento do mês.");
      }
      ...
      setIsLoadingFecharMes(false); // Desativa o estado de carregamento
      console.log("handleConfirmFecharMes finalizado."); // LOG PARA DEBUG
    }
  };
